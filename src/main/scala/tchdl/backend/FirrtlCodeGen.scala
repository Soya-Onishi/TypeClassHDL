...
object FirrtlCodeGen {
  ...
  def runCallBuiltIn(call: backend.CallBuiltIn)(implicit stack: StackFrame, ctx: FirrtlContext, global: GlobalData): RunResult = {
    ...
    call.label match {
      case "add_int_int" => builtin.intAdd(insts(0), insts(1), global)
      case "sub_int_int" => builtin.intSub(insts(0), insts(1), global)
      case "mul_int_int" => builtin.intMul(insts(0), insts(1), global)
      case "div_int_int" => builtin.intDiv(insts(0), insts(1), global)
      case "equal_int_int" => builtin.intEq(insts(0), insts(1), global)
      case "notEqual_int_int" => builtin.intNe(insts(0), insts(1), global)
      case "greaterThan_int_int" => builtin.intGt(insts(0), insts(1), global)
      case "greaterEqual_int_int" => builtin.intGe(insts(0), insts(1), global)
      case "lessThan_int_int" => builtin.intLt(insts(0), insts(1), global)
      case "lessEqual_int_int" => builtin.intLe(insts(0), insts(1), global)
      case "neg_int" => builtin.intNeg(insts(0), global)
      case "not_int" => builtin.intNot(insts(0), global)
      case "equal_bool_bool" => builtin.boolEq(insts(0), insts(1), global)
      case "notEqual_bool_bool" => builtin.boolNe(insts(0), insts(1), global)
      case "not_bool" => builtin.boolNot(insts(0), global)
      case "add_bit_bit" => builtin.bitAdd(insts(0), insts(1))
      case "sub_bit_bit" => builtin.bitSub(insts(0), insts(1))
      case "mul_bit_bit" => builtin.bitMul(insts(0), insts(1))
      case "div_bit_bit" => builtin.bitDiv(insts(0), insts(1))
      case "equal_bit_bit" => builtin.bitEq(insts(0), insts(1), global)
      case "notEqual_bit_bit" => builtin.bitNe(insts(0), insts(1), global)
      case "greaterThan_bit_bit" => builtin.bitGt(insts(0), insts(1), global)
      case "greaterEqual_bit_bit" => builtin.bitGe(insts(0), insts(1), global)
      case "lessThan_bit_bit" => builtin.bitLt(insts(0), insts(1), global)
      case "lessEqual_bit_bit" => builtin.bitLe(insts(0), insts(1), global)
      case "neg_bit" => builtin.bitNeg(insts(0))
      case "not_bit" => builtin.bitNot(insts(0))
      case "truncate_bit" => builtin.bitTruncate(insts(0), call.hargs(0), call.hargs(1), global)
      case "bit_bit" => builtin.bitBit(insts(0), call.hargs(0), global)
      case "concat_bit_bit" => builtin.bitConcat(insts(0), insts(1), global)
      case "idx_vector" => builtin.vecIdx(insts(0), call.hargs(0), global)
      case "idxDyn_vector_bit" => builtin.vecIdxDyn(insts(0), insts(1), global)
      case "updated_vector_*" => builtin.vecUpdated(insts(0), insts(1), call.hargs(0))
      case "updatedDyn_vector_bit_*" => builtin.vecUpdatedDyn(insts(0), insts(1), insts(2))
      case "from_bit_int" => builtin.bitFromInt(call.accessorTpe.get, insts(0))
      case "from_bit_bool" => builtin.bitFromBool(call.accessorTpe.get, insts(0))
      case "from_bit_bit" => builtin.bitFromBit(call.accessorTpe.get, insts(0))
    }
  }
